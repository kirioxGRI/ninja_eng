...
function AddWordCard() {
  const [form, setForm] = useState({
    palabra: '', pronunciacion: '', traduccion_espanol: '', oracion_ejemplo: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.error) {
      ...
      setForm({ palabra: '', pronunciacion: '', traduccion_espanol: '', oracion_ejemplo: '' });
    }
    setLoading(false);
  };
  ...
  return (
    <div className="progress-card" style={{ maxWidth: 520 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <input name="palabra" className="form-input" style={{ marginBottom: 0 }}
          placeholder="Palabra en inglés *" value={form.palabra} onChange={handleChange} disabled={loading} />
        <input name="pronunciacion" className="form-input" style={{ marginBottom: 0 }}
          placeholder="Pronunciación (ej: abándon)" value={form.pronunciacion} onChange={handleChange} disabled={loading} />
        <input name="traduccion_espanol" className="form-input" style={{ marginBottom: 0 }}
          placeholder="Traducción en español *" value={form.traduccion_espanol} onChange={handleChange} disabled={loading} />
        <textarea name="oracion_ejemplo" className="form-input" style={{ marginBottom: 0, resize: 'vertical', minHeight: 80 }}
          placeholder="Oración de ejemplo" value={form.oracion_ejemplo}
        ...
        <button id="btn-add-word" type="submit" className="btn btn-primary" disabled={loading}
  ...
  return (
    <div className="progress-card" style={{ maxWidth: 520 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <button id="btn-create-user-config" type="submit" className="btn btn-primary"
...
function CreateRoleCard() {
  return (
    <div className="progress-card" style={{ maxWidth: 520, opacity: 0.75 }}>
      ...
      <div style={{
        background: '#e3f2fd', borderRadius: 10, padding: '16px 20px',
        border: '1px dashed #90caf9',
      }}>
        <p style={{ color: '#1565c0', fontSize: '0.88rem', lineHeight: 1.6 }}>
          🚧 <strong>Disponible en una versión futura.</strong><br />
