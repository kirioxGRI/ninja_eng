...
import type { LearnedWordItem } from '@/app/actions/word-actions';
import { getLearnedWords, resetAllLearnedWords, unmarkWordAsLearned } from '@/app/actions/word-actions';
...
export default function LearnedWordsModal({ usuarioId, onClose, onStatsChange }: Props) {
  const [words, setWords] = useState<LearnedWordItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [confirmReset, setConfirmReset] = useState(false);
  const [isPending, startTransition] = useTransition();

  useEffect(() => {
    getLearnedWords(usuarioId).then((w) => {
      setWords(w);
      setLoading(false);
    });
  }, [usuarioId]);
  ...
  const handleRemove = (palabraId: number) => {
    startTransition(async () => {
      const res = await unmarkWordAsLearned(usuarioId, palabraId);
      if (!res.error) {
  ...
  const handleReset = () => {
    startTransition(async () => {
      const res = await resetAllLearnedWords(usuarioId);
      if (!res.error) {
        setWords([]);
        onStatsChange(0);
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.82rem', margin: '4px 0 0' }}>
              {loading ? 'Cargando...' : `${words.length} ${words.length === 1 ? 'palabra' : 'palabras'}`}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', color: '#90a4ae', padding: '40px 0' }}>
              ...
              <p style={{ fontWeight: 600 }}>No hay palabras aprendidas</p>
            </div>
          ) : (
            <div style={{
              display: 'grid',
                ...
                >
                  <div style={{ minWidth: 0 }}>
                    <div style={{
                      ...
                      color: '#0d47a1', whiteSpace: 'nowrap',
          ...
          {confirmReset ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }}>
              <span style={{ fontSize: '0.85rem', color: '#c62828', fontWeight: 600 }}>
                ¿Resetear todas las palabras aprendidas?
              </span>
              <button
                onClick={handleReset}
                disabled={isPending}
